		...
		for(let k in myObject)
		{
			if(myObject[k].filter_id===filter_id)
			{
					...
					for(let i in myBlocks)
					{
						...
						myBlocks[i].style.backgroundColor="yellow" 
						}						
					}
					...
					if(!(String(myCartButton[j].innerHTML)==="undefined")&&(typeof(myCartButton[j])==="object"))
						...
						myCartButton[j].style.backgroundColor="red"
...
function displayProduct(product)
{
	...
	<span>₹${product.price}</span><button class="float-right btn btn-info" onclick="addToCart(${product.product_id},'${product.product_name}','${product.imgSrc}',${product.price})">Buy now <i class="bi bi-bag-fill"></i></button></div>`
}
...
function proceedPayment(total)
{
	...
	// bankInfo.style.position = "relative";
	// bankInfo.style.zIndex = "9999";
	// bankInfo.style.padding = "20px";
	// bankInfo.style.margin = "20px auto";
	// bankInfo.style.width = "450px";

	let cardDiv=`<img src="../Images/gpay.jpg"><img src="../Images/paypal.jpg"><img src="../Images/rupay.jpg"><img src="../Images/mastercard.jpg">`
